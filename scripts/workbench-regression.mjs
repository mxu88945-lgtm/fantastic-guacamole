...
ok(sw.includes('const CACHE = "role-chat-cache-v127";'), "service worker cache was not bumped to v127");
...
const documentText = vm.runInContext(`workbenchDocument({html:'<h1>x</h1>',css:'h1{color:red}',js:'console.log("ok")'})`, context);
ok(documentText.includes("</script><script>console.log"), "generated sandbox document has broken script boundaries");
